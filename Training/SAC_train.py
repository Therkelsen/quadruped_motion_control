...
def main():
    ...
    num_envs = 4  # SAC learns efficiently with a few parallel envs
    ...
    vec_env = VecNormalize(
        ...
        norm_reward=True,
        clip_obs=10.0,
        reward_scale=20.0,  # scale up small rewards for better critic gradients
    )

    # 💾 Save normalization stats immediately (in case training is interrupted)
    vec_env.save(os.path.join(log_dir, "vecnormalize.pkl"))

    # ✅ Define SAC (Stable-Baselines3)
    ...
    # Set target entropy proportional to action dimension
